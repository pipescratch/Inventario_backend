...
function textoWhatsApp(grupo) {
  const lineas = grupo.items.map(
    (it, i) => `${i + 1}. ${it.nombre}\n   ${it.cantidad} un. — $${Math.round(it.total).toLocaleString("es-CO")}`
  );
  return (
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "32px 20px" }}>
        <div style={{ marginBottom: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px" }}>
            ...
            <span style={{ color: colores.texto, fontWeight: 600 }}>Historial</span>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                      ...
                      <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: `1px solid ${colores.borde}` }}>
                        ...
                        <div style={{ display: "grid", gap: "6px", marginTop: "8px" }}>
                          {agruparPorProveedor(lineas).map((grupo) => {
                            const proveedorInfo = proveedores.find((pv) => pv.nombre === grupo.proveedorNombre);
                            const telefono = proveedorInfo ? proveedorInfo.telefono : null;
                            return (
                              <div key={grupo.proveedorNombre} style={{ display: "flex", gap: "6px" }}>
                                <button
                                ...
                                >
                                  {copiado === p.id + grupo.proveedorNombre ? "✓ Copiado" : "Copiar"}
                                </button>
                                <a
                                  href={enlaceWhatsApp(textoWhatsApp(grupo), telefono)}
                                  target="_blank"
                                ...
                                >
                                  📱 {telefono ? "Enviar a " + grupo.proveedorNombre : "Enviar WhatsApp"}
                                </a>
                              </div>
                            );
                          })}
                    ...
                    {!pendiente && (
                      ...
                      >
                        {eliminandoId === p.id ? "Borrando..." : "Borrar pedido"}
                      </button>
                    )}
                  </div>
                );
